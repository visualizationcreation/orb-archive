...
for(const orb of manifest.orbs){
 if(orb.status==='inactive')throw new Error('Inactive projects belong in Archived Orbs (Inactive), not the public catalog');
 ...
 for(const edition of orb.editions){if(edition.status==='inactive')throw new Error('Inactive editions belong in Archived Orbs (Inactive)');if(!edition.id||editionIds.has(edition.id))throw new Error('Missing or duplicate edition ID');editionIds.add(edition.id);if(new URL(edition.url).protocol!=='https:')throw new Error('Edition URLs must use HTTPS');}
...
replace(/(<p class="results-label"[^>]*>)[^<]*(<\/p>)/,'<p class="results-label" id="count" role="status" aria-live="polite">'+manifest.orbs.length+' '+(manifest.orbs.length===1?'orb':'orbs')+' · '+editionIds.size+' '+(editionIds.size===1?'edition':'editions')+'</p>');
