(function(root,factory){
  ...
  function extras(orb){const h=holdings(orb);if(!h.library.length&&!h.audio.length)return '';return '<details class="museum-holdings"><summary>Reality Gallery & recordings <span>'+e([h.library.length?h.library.length+' library '+(h.library.length===1?'item':'items'):'',h.audio.length?h.audio.length+' '+(h.audio.length===1?'recording':'recordings'):''].filter(Boolean).join(' · '))+'</span></summary><div class="holdings-grid">'+(h.library.length?'<section><h3>Reality Gallery & sources</h3>'+links(h.library)+'</section>':'')+(h.audio.length?'<section><h3>Listen to this edition</h3>'+links(h.audio)+'</section>':'')+'</div></details>';}
