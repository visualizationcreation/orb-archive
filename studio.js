...
function project(v){const x=v.x*Math.cos(yaw)+v.z*Math.sin(yaw),z=v.z*Math.cos(yaw)-v.x*Math.sin(yaw),y=v.y*Math.cos(pitch)-z*Math.sin(pitch),zz=z*Math.cos(pitch)+v.y*Math.sin(pitch);const radius=Math.min(width*.355,height*.283),cx=width*.5,cy=height*.435;return {x:cx+x*radius,y:cy-y*radius,z:zz,cx,cy,radius};}
...
function renderIndex(){const query=$('point-search').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();const found=route.filter(p=>(p.label+' '+p.view+' '+p.tags.join(' ')).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query));
 ...
 $('point-index').replaceChildren(...found.map(p=>{const b=node('button');b.append(node('span',String(route.indexOf(p)+1).padStart(2,'0')),node('span',p.label));b.dataset.point=p.id;b.setAttribute('aria-current',String(selected?.id===p.id));b.onclick=()=>{select(p);if(innerWidth<761)$('reading-title').scrollIntoView({behavior:reduced.matches?'instant':'smooth',block:'start'})};return b}));
...
canvas.addEventListener('keydown',e=>{if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();select(route[Math.max(0,Math.min(route.length-1,route.indexOf(selected)+(e.key==='ArrowRight'?1:-1)))])}else if(e.code==='Space'){e.preventDefault();spinning=!spinning;updateMotion()}});
