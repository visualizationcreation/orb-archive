...
export async function buildMuseumPackage(input, options = {}) {
  ...
  const museum={attribution,...(attribution==='named'?{creator:{displayName:creatorName,creditType:'self_asserted_display_name',verified:false}}:{}),...(note||expression?{contribution:{...(note?{note}:{}),...(expression?{expression}:{})}}:{}),library,audio:record?.museum?.audio??[]};
