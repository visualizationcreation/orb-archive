...
(()=>{
  ...
  function apply(root=document.body){
    ...
    while(n=walker.nextNode()){if(n.parentElement.closest('script,style,code,textarea,.version-body,.community-row,.language-options'))continue;let original=originals.get(n);if(original===undefined){original=n.nodeValue;originals.set(n,original)}const value=language==='es'?translate(original):original;if(n.nodeValue!==value)n.nodeValue=value;}
    ...
    document.querySelectorAll('a[href]').forEach(a=>{const u=new URL(a.href,location.href);if(u.origin===location.origin&&u.pathname.startsWith(new URL('./',location.href).pathname)&&/\/(?:index\.html|about\.html|start\.html)?$/.test(u.pathname)&&!a.dataset.language){u.searchParams.set('lang',language);a.href=u.href}});
