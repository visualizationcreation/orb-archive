...
Studio gives the reading/media window most of the desktop width, beside a compact sticky sphere and control pad. On phones a 174px navigation dock stays above the reading while scrolling within the workspace. Display options are tucked into a disclosure. The cross follows authored Up/Down and neighboring Left/Right links. Home returns to the subject. Back/Forward retain their authored thread meanings; Previous/Next traverse the display route. Missing directions stay disabled. Canvas arrow keys use the same conceptual directions; Home returns to the subject. Selecting a new point brings its reading header back into view when necessary while retaining the navigation controls. The reading area has no nested scrolling pane. `studio-cockpit.css` supplies this reading-first layout after the base Studio stylesheet; preserve both.
